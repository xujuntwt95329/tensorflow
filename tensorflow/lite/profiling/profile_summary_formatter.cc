...
void ProfileSummaryProtoFormatter::GenerateOpProfileDataFromDetail(
  ...
  op_profile_data->set_first_inference_microseconds(
      detail->elapsed_time.first());
  op_profile_data->set_avg_inference_microseconds(detail->elapsed_time.avg());
  op_profile_data->set_times_called(detail->times_called /
                                    stats_calculator->num_runs());
  op_profile_data->set_mem_kb(detail->mem_used.newest() / 1000.0);
