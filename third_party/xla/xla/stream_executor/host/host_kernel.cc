...
SE_HOST_KernelError* HostKernel::worker(
    uint64_t block_size, uint64_t starting_i, SE_HOST_KernelThreadDim& max_dims,
  ...
  uint64_t y = sy;
  uint64_t x = sx;
  for (; z < max_dims.z && c > 0 && res == nullptr; ++z) {
    for (; y < max_dims.y && c > 0 && res == nullptr; ++y) {
      for (; x < max_dims.x && c > 0 && res == nullptr; ++x, --c) {
        SE_HOST_KernelThread kernel_thread = {x, y, z};
        SE_HOST_KernelCallFrame call_frame = {&max_dims, &kernel_thread,
                                              args.size(), args.data()};
        res = (*kernel_)(&call_frame);
      }
      x = 0;
    }
    y = 0;
...
absl::Status HostKernel::Launch(
  ...
  for (uint64_t i = 0; i < num_partitions; ++i) {
    ...
    auto func = [this, block_size, starting_i, &kernel_thread_dims, &args, &bc,
                 status]() {
      *status = worker(block_size, starting_i, kernel_thread_dims, args);
      bc.DecrementCount();
    };
  ...
  if (remainder != 0) {
    ...
    reminder_status = worker(remainder, starting_i, kernel_thread_dims, args);
  }
