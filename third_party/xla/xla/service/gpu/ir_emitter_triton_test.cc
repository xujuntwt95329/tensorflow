       ...
       backend_config={
         "fusion_backend_config":{
           "kind":"__triton_gemm","triton_gemm_config":{
             "block_m":"32","block_n":"32","block_k":"32","split_k":"1","num_stages":"1","num_warps":"4"}}}
})";
...
CHECK-DAG:   %[[CLAMPED_START_INDEX0:.*]] = arith.minsi %[[START_INDEX0]], %[[C3_i32]] : i32
...
TEST_F(TritonGemmTest, DynamicSliceIsSupportedInLhs) {
  // The select is used to restrict the start index to values that make sense.
  // It is not strictly needed, because we also support clamping the indices.
  // The start index(es) for the non-majormost dimension(s) are constant zero(s)
  // because we don't support dynamic slice on those dimensions.
  const std::string hlo_text = R"(
HloModule m
...
ENTRY e {
  ...
  EXPECT_THAT(
      module->entry_computation()->root_instruction(),
      GmockMatch(m::Fusion(m::Parameter(), m::Fusion(m::Parameter()),
                           m::Constant(), m::Parameter())
                     .WithFusionKind(HloInstruction::FusionKind::kCustom)));
  EXPECT_TRUE(RunAndCompare(hlo_text, ErrorSpec{/*aabs=*/1e-4, /*arel=*/1e-6}));
}

TEST_F(TritonGemmTest, DynamicSliceIsSupportedWhenTheStartIndexNeedsClamping) {
  // We use a select to get a number which is either 998 or 999 but not
  // constant. If it was constant, then the dynamic-slice would be optimized to
  // slice. The index will be clamped to 3 internally.
  // The start index(es) for the non-majormost dimension(s) are constant zero(s)
  // because we don't support dynamic slice on those dimensions.
  const std::string hlo_text = R"(
HloModule m
...
ENTRY e {
  ...
  dynamic_slice_input = f32[4,5,2] parameter(1)
  pred0 = pred[] parameter(2)
  c998 = s32[] constant(998)
  c999 = s32[] constant(999)
  start_index0 = s32[] select(pred0, c998, c999)
  start_index1 = s32[] constant(0)
  start_index2 = s32[] constant(0)
  dynamic_slice = f32[1,5,2] dynamic-slice(dynamic_slice_input, start_index0, start_index1, start_index2), dynamic_slice_sizes={1,5,2}
  reshape = f32[5,2] reshape(dynamic_slice)
  ROOT dot = f32[4,5] dot(dot_lhs, reshape), lhs_contracting_dims={0}, rhs_contracting_dims={1}
})";
...
ENTRY e {
  ...
  start_index1 = s32[] constant(0)
  start_index2 = s32[] constant(0)
  dynamic_slice = f32[1,5,2] dynamic-slice(
                    dynamic_slice_input, start_index0, start_index1, start_index2),
