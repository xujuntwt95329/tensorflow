...
TEST_F(ShapeInferenceTest, UnboundedFftWithFFT) {
  TF_ASSERT_OK_AND_ASSIGN(const Shape operand, ParseShape("c64[?, 10]"));
  const std::vector<int64_t> fft_length = {3, 9};
  TF_ASSERT_OK_AND_ASSIGN(const Shape expected, ParseShape("c64[?, 10]"));
  TF_ASSERT_OK_AND_ASSIGN(const Shape inferred_shape,
...
TEST_F(ShapeInferenceTest, UnboundedFftWithRFFT) {
  TF_ASSERT_OK_AND_ASSIGN(const Shape operand, ParseShape("f64[10, ?]"));
...
TEST_F(ShapeInferenceTest, UnboundedFftWithIRFFT) {
  TF_ASSERT_OK_AND_ASSIGN(const Shape operand, ParseShape("c128[10, ?]"));
  const std::vector<int64_t> fft_length = {5};
